...
#### 4.2 Connect Microsoft Power BI to Azure Databricks Using SQL Warehouse

Power BI is connected to Databricks using the **Databricks SQL Warehouse (Serverless)** connection.

**Steps followed:**

####  1. Open **Power BI Desktop**
#### 2. Click **Get Data → Azure → Azure Databricks**
#### 3. Enter the following details obtained from Databricks:
   - **Server Hostname**
   - **HTTP Path**  
   (Found under **Databricks → SQL Warehouses → Serverless → Connection Details**)
#### 4. Select **Microsoft Account** as the authentication method
#### 5. Click **Sign In** to proceed

This establishes a secure live connection between Power BI and Databricks without exporting data manually.

![Alt text for the image](Screenshots/sql_warehouse_connection.png)
![Alt text for the image](Screenshots/powerbi_connection.png)
...
![Alt text for the image](Screenshots/powerbi_tables.png)
